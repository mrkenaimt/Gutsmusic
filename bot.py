        ...
        try:
            ...
            try:
                ...
                with yt_dlp.YoutubeDL(diag_opts) as ydl2:
                    info2 = ydl2.extract_info(f"https://www.youtube.com/watch?v={vid}", download=False, process=False)
                    info2 = ydl2.process_ie_result(info2, download=False)
                    fmts = info2.get('formats', [])
                    audio_fmts = [f"{f.get('format_id')}:{f.get('ext')}:{f.get('acodec')}" for f in fmts if f.get('acodec') and f.get('acodec') != 'none']
                    diag = f"\n\nإجمالي صيغ: {len(fmts)} | صيغ صوتية: {audio_fmts[:8]}"
