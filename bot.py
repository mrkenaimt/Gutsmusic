...
# تحديث yt-dlp تلقائياً عند بدء التشغيل (لتجنب مشاكل كاش البناء على Railway)
try:
    subprocess.run(
...
if not BOT_TOKEN:
    raise SystemExit("❌ تأكدي من ضبط BOT_TOKEN في Variables على Railway")
...
# ====== نفس إعدادات yt-dlp البسيطة من working1.py (المُختبرة وتعمل) ======
ydl_opts = {
...
# نفس إعدادات البحث من working1.py
search_opts = {'quiet': True, 'default_search': 'ytsearch10'}

# نفس إعدادات استخراج المعلومات (للعنوان) من working1.py
info_opts = {'quiet': True}

# لو فيه كوكيز مرفوعة، نضيفها لكل الإعدادات الثلاثة كاحتياط فقط
...
# ====== البحث المباشر بالرسالة (ستايل bot.py) لكن بمنطق working1.py ======
...
# ====== معالجة الأزرار (ستايل bot.py) ======
@bot.callback_query_handler(func=lambda call: True)
def callback(call):
    chat_id = call.message.chat.id
    data = call.data

    # --- تحميل MP3 (نفس منطق working1.py بالضبط) ---
    ...
    # --- جلب الكلمات (نفس منطق working1.py في استخراج العنوان) ---
