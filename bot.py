...
# تحديث yt-dlp تلقائياً عند بدء التشغيل (تجاوز لمشاكل كاش البناء على Railway)
try:
    subprocess.run(
...
def callback(call):
    ...
    if data.startswith("dl|"):
        ...
        try:
            print(f"[DL] بدء التحميل لـ {vid}")
            video_title, duration = get_video_info(vid)
            print(f"[DL] العنوان: {video_title} | المدة: {duration}")

            url = f"https://www.youtube.com/watch?v={vid}"
            print(f"[DL] خيارات yt-dlp: {YDL_DOWNLOAD_OPTS}")
            with yt_dlp.YoutubeDL(YDL_DOWNLOAD_OPTS) as ydl:
                ydl.extract_info(url, download=True)
            print("[DL] التحميل نجح")
            ...
            diag = ""
            try:
                simple_opts = {'quiet': False, 'no_warnings': False, 'listformats': False}
                if os.path.exists('cookies.txt'):
                    simple_opts['cookiefile'] = 'cookies.txt'
                with yt_dlp.YoutubeDL(simple_opts) as ydl2:
                    info2 = ydl2.extract_info(f"https://www.youtube.com/watch?v={vid}", download=False)
                fmts = info2.get('formats', []) if info2 else []
                print(f"[DIAG] عدد الصيغ الكلي: {len(fmts)}")
                audio_fmts = []
                for f in fmts:
                    acodec = f.get('acodec')
                    if acodec and acodec != 'none':
                        audio_fmts.append(f"{f.get('format_id')}:{f.get('ext')}:{acodec}")
                print(f"[DIAG] صيغ صوتية: {audio_fmts}")
                diag = f"\n\nإجمالي صيغ: {len(fmts)} | صوتية: {audio_fmts[:8]}"
            except Exception as e2:
                print(f"[DIAG] فشل التشخيص: {e2}")
                diag = f"\n\n(فشل التشخيص: {str(e2)[:150]})"

            safe_msg = f"❌ خطأ في التحميل:\n{str(e)[:200]}{diag}"
            try:
                bot.send_message(chat_id, safe_msg)
            except Exception:
                bot.send_message(chat_id, "❌ خطأ في التحميل (تعذر عرض التفاصيل).")
